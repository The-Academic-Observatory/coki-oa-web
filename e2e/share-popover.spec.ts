...
test("Should check that social share links open new tabs with correct URLs", async ({ page, isMobile, context }) => {
  ...
  expect(newTab.url()).toEqual(
    "https://twitter.com/intent/tweet?text=Check%20out%20the%20Open%20Access%20statistics%20for%20Mali%20on%20the%20COKI%20Open%20Access%20Dashboard%3A&url=http%3A%2F%2Flocalhost%3A3000%2Fcountry%2FMLI%2F&hashtags=OpenAccess,COKI&via=COKIproject",
  );
  await newTab.close();
  ...
  expect(params.get("session_redirect")).toEqual(
    "https://www.linkedin.com/shareArticle?mini=true&url=http%3A%2F%2Flocalhost%3A3000%2Fcountry%2FMLI%2F",
  );
  await newTab.close();

  newTab = await openShareLink("share-facebook");
  params = new URLSearchParams(new URL(newTab.url()).search);
  let expected = "https://www.facebook.com/sharer/sharer.php?u=http%3A%2F%2Flocalhost%3A3000%2Fcountry%2FMLI%2F";
  if (isMobile) {
    expected = "https://m.facebook.com/sharer/sharer.php?u=http%3A%2F%2Flocalhost%3A3000%2Fcountry%2FMLI%2F";
  }
