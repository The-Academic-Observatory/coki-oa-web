...
test("Should check that social share links open new tabs with correct URLs", async ({ page, isMobile, context }) => {
  ...
  expect(newTab.url()).toEqual(
    "https://twitter.com/intent/tweet?text=Open%20Access%20statistics%20for%20Mali!&url=http://localhost:3000/country/MLI/",
  );
  await newTab.close();
  ...
  expect(params.get("session_redirect")).toEqual(
    "https://www.linkedin.com/shareArticle?mini=true&url=http://localhost:3000/country/MLI/",
  );
  await newTab.close();
